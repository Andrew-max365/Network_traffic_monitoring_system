# SentinelFlow: 网络流量分析与异常检测系统

[cite_start]**SentinelFlow** 是一款专为网络空间安全、信息安全及密码技术专业学生设计的综合实践平台 [cite: 3][cite_start]。系统采用 **C++ 核心算法层**与 **Python 现代图形界面层**相结合的架构，实现了从流量数据解析、复杂拓扑构建到路径拥塞分析与交互式可视化的全流程功能 [cite: 105-111]。

[cite_start]本项目为华中科技大学（HUST）《2024级程序设计综合课程设计》作品 [cite: 1-6]。
...
### 1. 深度流量审计 (C++ Engine)
* [cite_start]**高效图拓扑构建**：基于邻接表结构（Adjacency List）存储网络节点及会话信息 [cite: 60, 78-80][cite_start]，支持源/目的 IP、协议类型（TCP/UDP/ICMP）、数据大小及持续时间的完整记录 [cite: 69-75, 80]。
* [cite_start]**流量多维排序**：实现对全节点总流量的快速排序 [cite: 81-84]，并支持针对 HTTPS（TCP:443）协议的专项流量排行榜。
* **智能路径规划**：

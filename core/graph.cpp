...
void add_session(Graph *g, char *src, char *dst, int proto, long bytes, double duration) {
    ...
    while (p) {
        if (p->dest_idx == v) {
            ...
            p->duration += duration;
            if (proto == 6) {
                p->p_stats.tcp_bytes += bytes;
            } else if (proto == 17) {
                p->p_stats.udp_bytes += bytes;
            }
            p->p_stats.total_duration += duration;
            return;
        }
        p = p->next;
    }
    ...
    new_e->p_stats.tcp_bytes = (proto == 6) ? bytes : 0;
    new_e->p_stats.udp_bytes = (proto == 17) ? bytes : 0;
    new_e->p_stats.total_duration = duration;
